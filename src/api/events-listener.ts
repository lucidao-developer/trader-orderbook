...
export function startEventListeners() {
    ...
    const contract = new ethers.Contract(addresses[CHAIN_IDS.SKALE_TESTNET]?.exchange.toString()!, IZeroEx.compilerOutput.abi, provider);

    contract.on(contract.filters.ERC1155OrderFilled(), async (direction, maker, taker, nonce, erc20Token, erc20FillAmount, erc1155Token, erc1155TokenId, erc1155FillAmount, matcher) => {
    ...
    contract.on(contract.filters.ERC721OrderFilled(), async (direction, maker, taker, nonce, erc20Token, erc20TokenAmount, erc721Token, erc721TokenId, matcher) => {
